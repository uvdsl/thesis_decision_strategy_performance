...
for p in x: #list(map(lambda x: round(x,2),np.linspace(0.05,0.95,19)))
    ...
    df_sub = df_sub.append(df.sum()/df.shape[0], ignore_index=True)
...
plt.plot(x,df_sub['reach'], color='red', linewidth=3, marker='*', markersize=10)
plt.plot(x,df_sub['initH'], color='darkblue', linewidth=3, marker='o', markersize=10)
plt.plot(x,df_sub['iterH'], color='orange', linestyle='dashed', linewidth=2, marker='x', markersize=10)
plt.plot(x,df_sub['deg'], color='green', linewidth=3, marker='+', markersize=10)
...
plt.ylabel(r'\textit{Optimal Subset Rate}', fontsize=32)
plt.grid(True)
plt.tight_layout()

if not os.path.exists(f"../img/({n})/"):
        os.mkdir(f"../img/({n})/")
plt.savefig(f'../img/({n})/sub.pdf')
